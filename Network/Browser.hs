...
import Network.HTTP.Utils ( ConnError(..), Result )
...
anticipateChallenge :: Request -> BrowserAction t (Maybe Authority)
...
request :: Request
        -> BrowserAction Connection (URI,Response)
request req = nextRequest $ do
  res <- request' [] initialState req
...
request' :: String
         -> RequestState
         -> Request
         -> BrowserAction Connection (Result (URI,Response))
...
updateConnectionPool :: Connection
                     -> BrowserAction Connection ()
...
cleanConnectionPool :: URIAuth -> BrowserAction Connection ()
